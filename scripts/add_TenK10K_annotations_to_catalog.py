...
This script owns every TenK10K_* field: each run clears them first and then writes only the
...
def main():
    ...
    unlisted_fields = find_unlisted_annotation_fields(annotation_lookup, TENK10K_FIELDS)
    if unlisted_fields:
        parser.error(f"TENK10K_FIELDS doesn't list {sorted(unlisted_fields)}, so re-running this script "
    ...
    with fopen(args.catalog_json_path, "rt") as f:
        ...
        with f2open(args.output_catalog_json_path, "wt") as f2:
            ...
            for i, record in enumerate(iterator):
                ...
                had_previous_annotation = clear_previous_annotations(record, TENK10K_FIELDS)
    ...
    print_annotation_replacement_summary("TenK10K", f"are no longer in {args.tsv_path}",
