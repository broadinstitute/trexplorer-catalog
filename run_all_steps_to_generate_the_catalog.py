...
for key in "hg38_reference_fasta", "gencode_gtf", "variation_clusters_tsv", "tenk10k_annotations", "hprc256_annotations", "aou1027_annotations":
...
""", step_number=16)
    ...
    run(f"""python3 -u {base_dir}/scripts/add_allele_frequency_annotations.py \
            ...
            -o {annotated_catalog_path}.with_allele_frequencies.json.gz  {annotated_catalog_path}""", step_number=25)

    run(f"mv {annotated_catalog_path}.with_allele_frequencies.json.gz {annotated_catalog_path}", step_number=26)
    ...
    if args.tenk10k_annotations:
        run(f"""python3 {base_dir}/scripts/add_TenK10K_annotations_to_catalog.py \
            ...
            {annotated_catalog_path}""", step_number=27)
        run(f"mv {annotated_catalog_path}.with_TenK10K_annotations.json.gz {annotated_catalog_path}", step_number=28)
    ...
    if args.hprc256_annotations:
        run(f"""python3 {base_dir}/scripts/add_HPRC256_annotations_to_catalog.py \
            ...
            {annotated_catalog_path}""", step_number=29)
        run(f"mv {annotated_catalog_path}.with_HPRC256_annotations.json.gz {annotated_catalog_path}", step_number=30)
    ...
    if args.aou1027_annotations:
        run(f"""python3 {base_dir}/scripts/add_AoU_annotations_to_catalog.py \
            ...
            {annotated_catalog_path}""", step_number=31)
        run(f"mv {annotated_catalog_path}.with_AoU1027_annotations.json.gz {annotated_catalog_path}", step_number=32)
    ...
    run(f"python3 -m str_analysis.convert_expansion_hunter_catalog_to_bed --split-adjacent-repeats "
        f"{annotated_catalog_path}  --output-file {output_prefix}.bed.gz", step_number=33)
    ...
    run(f"python3 -m str_analysis.add_adjacent_loci_to_expansion_hunter_catalog "
        ...
        f"{annotated_catalog_path}", step_number=34)

    run(f"mv {annotated_catalog_path}.with_adjacent_loci_annotation.json.gz {annotated_catalog_path}", step_number=35)
...
""", step_number=36)
    ...
    run(f"python3 -m str_analysis.convert_expansion_hunter_catalog_to_trgt_catalog -R {args.hg38_reference_fasta} --split-adjacent-repeats {annotated_catalog_path}  --output-file {output_prefix}.TRGT.bed", step_number=37)
    run(f"python3 -m str_analysis.convert_expansion_hunter_catalog_to_longtr_format  {annotated_catalog_path}  --output-file {output_prefix}.LongTR.bed", step_number=38)
    run(f"python3 -m str_analysis.convert_expansion_hunter_catalog_to_hipstr_format  {annotated_catalog_path}  --output-file {output_prefix}.HipSTR.bed", step_number=39)
    run(f"python3 -m str_analysis.convert_expansion_hunter_catalog_to_gangstr_spec   {annotated_catalog_path}  --output-file {output_prefix}.GangSTR.bed", step_number=40)

    # Confirm that the TRGT catalog passes 'trgt validate'
    run(f"trgt validate --genome {args.hg38_reference_fasta}  --repeats {output_prefix}.TRGT.bed", step_number=41)

    # Print missing values
    run(f"python3 {base_dir}/scripts/print_missing_values_percentages_in_json_or_tsv.py  {annotated_catalog_path}", step_number=42)
    ...
    run(f"python3 {base_dir}/scripts/validate_catalog.py " +
        ...
        f"{annotated_catalog_path}", step_number=43)
        ...
        if path.endswith(".bed"):
            if not os.path.isfile(f"{path}.gz"):
                if path.endswith(".TRGT.bed"):
                    run(f"gzip -f {path}", step_number=44)  # TRGT v1.1.1 and lower only works with gzip, not bgzip
                else:
                    run(f"bgzip -f {path}", step_number=45)
    ...
    if release_tar_gz_path is None:
        for path in updated_release_files:
            run(f"cp {path} {release_draft_folder}", step_number=47)
    else:
        run(f"tar czf {release_tar_gz_path} -C {os.path.dirname(output_prefix)} " + " ".join([os.path.basename(p) for p in updated_release_files]), step_number=48)
        run(f"cp {release_tar_gz_path} {release_draft_folder}", step_number=49)

    run(f"python3 -m str_analysis.compute_catalog_stats --reference-fasta {args.hg38_reference_fasta} --verbose {annotated_catalog_path}", step_number=50)

    # Print source statistics table
    run(f"python3 {base_dir}/scripts/generate_catalog_sources_stats_table.py {release_draft_folder}/{os.path.basename(annotated_catalog_path)}", step_number=51)
    ...
    run(f"python3 -u -m str_analysis.convert_gangstr_spec_to_expansion_hunter_catalog --verbose {comparison_catalog_paths['GangSTR_v17']} -o {path_after_conversion}", step_number=52)
    ...
    for catalog_name, path in comparison_catalog_paths.items():
        ...
        run(f"python3 -m str_analysis.compute_catalog_stats --reference-fasta {args.hg38_reference_fasta} --verbose {filtered_comparison_catalog_path}", step_number=54)
        ...
        run(f"""python3 -u -m str_analysis.merge_loci \
            ...
            {filtered_comparison_catalog_path}""", step_number=55)
